...
set :application_name, 'yueqi'
set :domain, '47.104.57.110'
set :deploy_to, '/var/www/yqjxcx.org'
set :repository, 'git@gitlab.hf66.net:liuxuewen/yueqi.git'
set :branch, 'master'
#set :term_mode, :nil
# Optional settings:
...
namespace :puma do
  ...
  set :puma_config,    -> { "/var/www/yqjxcx.org/current/config/puma.rb" }
  set :puma_socket,    -> { "/var/www/yqjxcx.org/shared/tmp/sockets/puma.sock" }
  set :puma_state,     -> { "/var/www/yqjxcx.org/shared/tmp/sockets/puma.state" }
  set :puma_pid,       -> { "/var/www/yqjxcx.org/shared/tmp/pids/puma.pid" }
  set :puma_cmd,       -> { "puma" }
  set :pumactl_cmd,    -> { "pumactl" }
  set :pumactl_socket, -> { "/var/www/yqjxcx.org/shared/tmp/sockets/pumactl.sock" }
  # Make necessary direcotries exist
  task :setup => :environment do
    command "mkdir /var/www/yqjxcx.org/shared/tmp/sockets/"
    command "mkdir /var/www/yqjxcx.org/shared/tmp/pids/"
  end
    ...
    command %[
      if [ -e '#{fetch(:pumactl_socket)}' ]; then
        ...
        if [ -e '#{fetch(:puma_config)}' ]; then
          ...
          cd /var/www/yqjxcx.org/current && #{fetch(:puma_cmd)} -q -d -e #{fetch(:puma_env)} -b 'unix://#{fetch(:puma_socket)}' -S #{fetch(:puma_state)} --control 'unix://#{fetch(:pumactl_socket)}'
  ...
  task stop: :environment do
    command %[
      if [ -e '#{fetch(:pumactl_socket)}' ]; then
        cd /var/www/yqjxcx.org/current  && #{fetch(:pumactl_cmd)} -S #{fetch(:puma_state)} stop
        cd /var/www/yqjxcx.org/shared/tmp/sockets/ && rm puma.sock puma.state pumactl.sock
      else
